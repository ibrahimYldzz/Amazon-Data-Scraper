...
def create_webdriver() -> webdriver.Chrome:
    options = Options()
    options.headless = True  # Tarayıcıyı başsız modda çalıştırır
...
def run(search_term):
    ...
    driver = webdriver.Chrome(executable_path="./chromedriver.exe") ## add chromdriver
    num_records_scraped = 0

    for page in range(1, 6):  # max of 20 pages
        # load the next page
